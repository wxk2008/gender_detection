 ...
 */
 CvGabor::CvGabor(int iMu, int iNu, double dSigma, double dF)   //空间频率和sigma都是用户自己设置的。
{
 ...
 */
 CvGabor::CvGabor(double dPhi, int iNu)   //方向和尺度都是用户自己设置的，空间频率和sigma分别取：sqrt(2.0),2*PI
{
    Sigma = 2*PI;
    F = sqrt(2.0);
 ...
 */
 CvGabor::CvGabor(double dPhi, int iNu, double dSigma) //方向、尺寸大小和sigma都是用户自己设定，空间频率用缺省值sqrt(2.0)
{
 ...
 */
 CvGabor::CvGabor(double dPhi, int iNu, double dSigma, double dF)  //所有的都是用户自己设定。
{
 ...
 */
long CvGabor::mask_width()  //确定mask的宽度 （N*N窗口）
{
